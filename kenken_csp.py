...
def binary_ne_grid(kenken_grid):
    ...
    for i in list_vars:
        ...
        for j in list_vars:
            var2=j.name
            # if first integer is the same they are in the same row, and 2nd in the same column
            if var1 != var2:
...
def nary_ad_grid(kenken_grid):
    ...
    all_tuples = list(itertools.permutations(domain,N))

    # construct row and col constrains
    for i in range(1, N + 1, 1):
        row_name = "row #" + str(i)
        col_name = "column #" + str(i)
        row_cons = Constraint(row_name, all_row[i-1])
        col_cons = Constraint(col_name, all_col[i-1])
        row_cons.add_satisfying_tuples(all_tuples)
        col_cons.add_satisfying_tuples(all_tuples)
